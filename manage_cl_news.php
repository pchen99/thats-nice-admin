...
<body>
	<div id="wrapper">
    	...
    	<h1><a href="index.php"><span>Client News Admin</span></a></h1>
...
<?php include "share/home_nav.php"; ?>
                ...
                <h2><a href="#">Admin</a> &raquo; <a href="#" class="active">Client News</a></h2>
                
                <div id="main">
                <?php 
				
			if ($cat == 'hd') {
			echo "<h3>Add to Home Decor</h3>";
			} elseif ($cat == 'sm') { 
			echo "<h3>Add to Specialty Materials</h3>";
			}
?>

<?php echo $error; ?>
      
<form action="create_cl_news.php" method="post" enctype="multipart/form-data" name="form1" id="form1">

<fieldset>
    
      <p>Headline<br>
      <input type="text" name="headline" id="headline" class="text-long" />
      <input type="hidden" name="cat" id="cat" value="<?php echo $cat; ?>" class="text-long" /></p>
    ...
    <p>Witdh<br />
     ...
     <p><input name="button" type="submit" class="button-submit" id="button" value="Create" /></p>
...
<h3>Manage Client News</h3>
      
<p> News that is set to "active" will appear on the home page.</p>
   <br>
...
// -- Fetch the data --------//
$grab = mysql_query("SELECT * FROM TN_cl_news WHERE cat='$cat' ORDER BY id ASC");
...
{	
  ...
  echo "
    <tr>
      <td>$headline</td>
	  <td class=\"action\"><a href=\"edit_cl_news.php?id=$id&cat=$cat\" class=\"view\">Edit</a></td>
	  ";
	  
    if ($active == 1) {
	echo "<td class=\"action\"><a href='activate_cl_news.php?id=$id&cat=$cat&action=deactivate' style='color:green'>Active</a></td>";
	} else { 
	echo "<td class=\"action\"><a href=\"activate_cl_news.php?id=$id&cat=$cat&action=activate\">Select</a></td>";
	}
	
	echo "<td class=\"action\"><a href=\"delete_cl_news.php?id=$id&cat=$cat\" class=\"delete\">Delete</a></td>
	</tr>";  
} 
